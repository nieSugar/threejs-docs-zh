...
// Mark deprecated API names visually while keeping their replacement notes readable.
( function markDeprecatedMembers() {

	const tags = document.querySelectorAll( '.tag-deprecated' );

	for ( const tag of tags ) {

		const member = tag.closest( '.member' );

		if ( member ) {

			member.classList.add( 'deprecated-member' );
			continue;

		}

		const method = tag.closest( '.method' );
		const name = method?.previousElementSibling;

		if ( name?.matches( 'h3.name' ) ) name.classList.add( 'deprecated-name' );
